...
- 定义
    树可以递归定义为：树由结点的集合组成，集合可以为空对应空树，也可以为拥有一个根结点和零个或多个子树的集合，子树通过边与根结点相连
- 其他概念
  - 叶结点leaf：没有子树只有根结点的树结点称为树的叶结点
  - 兄弟结点siblings：拥有同一个根结点的子树根结点互为兄弟结点
  - 结点路径：从结点n1到结点nk的路径定义为一系列结点的串且后一个结点是前一个结点的子结点，路径长就是连接这一系列结点的边的数目k-1，树中从根结点到任意结点的路径唯一
  - 结点深度depth：结点深度就是从根结点到此结点唯一路径的长度，根结点深度为0
  - 结点高度height：结点高度就是从此结点到叶结点最长路径的长度，叶结点高度为0
  - 完全二叉树complete：除底层外每一层都满，底层从左到右填充，高度为⌊logN⌋
  - 完美二叉树perfect：有2^(h+1)-1个结点，完全填满每一层的二叉树
- 实现
    由于树具有多少个子结点是未知且不统一的，因此难以通过每个结点管理自身所有子结点来实现，而是通过每个结点保存下一个兄弟结点指针和第一个子结点指针来实现
- 遍历
  - 前序preorder：根->子节点
  - 后序postorder：子节点->根
...
任意结点最多拥有两个子结点的树，平均深度为logN，但是最坏情况下深度为N-1

## 3. 二叉搜索树 The Search Tree ADT—Binary Search Trees

- 定义
    任意结点左侧子树中存储的所有数据小于此结点数据，右侧子树中存储的所有数据大于此结点数据
- 操作
  - 包含contain
  - 最小值findMin/最大值findMax
  - 插入insert
  - 删除remove：
    - 可以用被删除数据左子树的最大值或右子树的最小值来代替被删除的结点，随后递归删除代替值的结点
    - 懒惰删除，结点被标记为删除，等并不从树中移除
...
- **维护平衡状态**
    需要在每个结点额外维护一个此结点的高度值，当插入元素时更新到根结点路径的高度，一旦出现高度超过1即执行平衡维护操作，空树的高度定义为-1
    ![4.3](images/4.3.png)
  ...
  - 插入后递归维护每个结点的高度，发现超过1即执行平衡操作，若新高度与旧高度一致则结束

## 5. 伸展树 Splay Trees

- 定义
    伸展树定义了一系列操作下的时间复杂度界限，如M次操作下的时间复杂度为O(MlogN)，一些操作可能多于O(logN)，一些少于O(logN)，最终平摊结果是O(logN)的操作复杂度。根据**局部性原理，一些结点被访问后很有可能在不久的未来被再次访问，因此可以通过将这次访问的结点提升到根附近来加速未来的访问**
- **伸展**
    通过AVL的操作也可以实现结点的提升，但是整体性能依然是下降的，由此我们采用另外的方式来实现结点的提升，即Zig-zag和Zig-zig，可以使被访问路径上的所有结点整体提升
  - Zig-zag（即AVL树的双旋转）
  - Zig-zig
  ![4.4](images/4.4.png)
- 操作
  - 插入
  - 删除
        对于伸展树删除元素，只需要访问一次被删除元素，使其被伸展至根结点，删除后就有了左子树和右子树，随后通过访问左子树的最大值（或右子树的最小值）使其位于子树的根结点，则这个新的根结点只有左子树，那么就可以将右子树作为新根的右子树，删除完成
  - 最大值/最小值
...
- 层序遍历（Levelorder traversal）：从根开始同一深度的结点连续扫描
...
- M叉B树的性质
  - 叶结点存储数据
  - 非叶结点存储最多M-1个关键字来引导查找，第i个关键字代表第i+1子树内的最小值
  - 树的根结点要么没有子树（叶结点），要么拥有2到M个子结点
  - 除根结点以外的非叶结点有「M/2到M个子结点
  - 所有叶结点深度相同，且拥有「L/2到L个数据，L为B树的阶数
- 操作
    由于需要保持B树的性质，因此存在合并兄弟结点和拆分兄弟结点的操作，参考右侧图示，当插入导致单个叶子结点无法容纳数据时就近似等分成两个叶结点，同理当非叶结点容纳不下时也近似等分，递归直到根结点，根节点容纳不下时就近似等分成两个非叶结点，并且一个新根结点出现，因此根结点允许最少2个子结点（此时B树增加一层高度），当相邻两个结点不足半满时就自动合并类似拆分的逆动作
